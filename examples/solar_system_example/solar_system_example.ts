...
(async () => {
  ...
  data.bodies.forEach((body: any) => {
    if (body.isPlanet as boolean) {
  ...
  const radiusSeries = new cop.elements.PieSeries(
    planets.asDict().planets.map((planet: any) => planet.name as string),
    planets.asDict().planets.map((planet: any) => planet.equaRadius as number),
    "radius",
    color
  );
