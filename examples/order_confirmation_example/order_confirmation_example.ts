import * as aop from '../../src/index';

const TEMPLATE_PATH = './examples/order_confirmation_example/data/template.docx';
const SERVER_URL = 'https://api.apexofficeprint.com/';
const API_KEY = 'YOUR_API_KEY'; // Replace by your own API key

const template = aop.Resource.fromLocalFile(TEMPLATE_PATH);

const server = new aop.config.Server(
    SERVER_URL,
    new aop.config.ServerConfig(API_KEY),
);

// Main ElementCollection that includes all the data
const data = new aop.elements.ElementCollection('data');

// Company information
const companyName = new aop.elements.Property('company_name', 'APEXOfficePrint');
const companyLogo = aop.elements.Image.fromFile('company_logo', './examples/order_confirmation_example/data/logo-office-print.jpg');
companyLogo.maxHeight = 200;
companyLogo.maxWidth = 200;

// / Add company information to data
data.add(companyName);
data.add(companyLogo);

// Customer information
const cust = aop.elements.ElementCollection.fromMapping({
    cust_city: 'St. Louis',
    cust_first_name: 'Albertos',
    cust_last_name: 'Lambert',
});

// / Add customer information to data
data.addAll(cust);

// Order information

// / Order 1
const order1 = aop.elements.ElementCollection.fromMapping({
    order_name: 'Order 1',
    order_total: 610,
    ref_nb: 5645,
});

// // Add products for first order

// /// First product for first order
let product1 = aop.elements.ElementCollection.fromMapping({
    product_name: 'Business \nShirt',
    quantity: 3,
    unit_price: 50,
    in_stock: 3,
});

// //// Add image for first product
let image = aop.elements.Image.fromBase64('image', '');
product1.add(image);

// /// Second product of first order
let product2 = aop.elements.ElementCollection.fromMapping({
    product_name: 'Trousers',
    quantity: 2,
    unit_price: 80,
    in_stock: 1,
});

// //// Add image for second product
image = aop.elements.Image.fromBase64('image', '\r\nIiAiIgIiIP/Z');
product2.add(image);

// /// Third product of first order
const product3 = aop.elements.ElementCollection.fromMapping({
    product_name: 'Jacket',
    quantity: 2,
    unit_price: 150,
    in_stock: 3,
});

// //// Add image for third product
image = aop.elements.Image.fromBase64('image', '\r\n4c/V7XFzuXsj8tVfx+ClNEQEREBERAREQEREBERAREQEREBERAREQf/Z');
product3.add(image);

// /// Add products to order1
let products = new aop.elements.ForEach('product', [product1, product2, product3]);
order1.add(products);

// / Order 2
const order2 = aop.elements.ElementCollection.fromMapping({
    order_name: 'Order 2',
    order_total: 340,
    ref_nb: 4561,
});

// // Add products for second order

// /// First product of second order
product1 = aop.elements.ElementCollection.fromMapping({
    product_name: 'Blouse',
    quantity: 3,
    unit_price: 60,
    in_stock: 10,
});

// //// Add image for first product
image = aop.elements.Image.fromBase64('image', '\r\n0ERFIcwIiIAiIgCIiAIiIAiIgCIiAIiIAiIgP//Z');
product1.add(image);

// /// Second product of second order
product2 = aop.elements.ElementCollection.fromMapping({
    product_name: 'Skirt',
    quantity: 2,
    unit_price: 80,
    in_stock: 1,
});

// //// Add image for second product
image = aop.elements.Image.fromBase64('image', '');
product2.add(image);

// /// Add products to order2
products = new aop.elements.ForEach('product', [product1, product2]);
order2.add(products);

// / Add orders to an object list
const orders = new aop.elements.ForEach('orders', [order1, order2]);

// / Add orders to data
data.add(orders);

// Merge template and data to generate the output file
const conf = new aop.config.OutputConfig('pdf'); // Optional
const printjob = new aop.PrintJob(data, server, template, conf);

(async () => {
    try {
        const res = await printjob.execute();
        res.toFile('./examples/order_confirmation_example/output/output'); // Save response to output file
    } catch (err) {
        throw new aop.exceptions.AOPError(err);
    }
})();
